...
export default function ScheduleModal({ isOpen, onClose }: ScheduleModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setFeedbackMessage({ type: 'success', message: 'Schedule saved successfully!' });
              ...
              >
                <div className="flex space-x-4 bg-[#EBE5D3] p-1 rounded-full">
                  {['daily', 'custom'].map((type) => (
                    <button
                      key={type}
                      onClick={() => setScheduleType(type as 'daily' | 'custom')}
                ...
                <div className="relative" ref={timeDropdownRef}>
                  ...
                  >
                    <Clock size={20} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#5D552F]" />
                    <span>{time}</span>
                    {isTimeDropdownOpen ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                  </div>
                  {isTimeDropdownOpen && (
                ...
                {feedbackMessage && (
                  ...
                  >
                    {feedbackMessage.type === 'success' && <Check size={16} className="mr-2" />}
